  ...
  while (i < lastPrices.length) {
    ...
    if (newPrice >= minPricePositiv) {
      ...
      if (newPrice > currentPair.initialPrice) {
        ...
        // variableHistory.moreThen = pairs[lastPrices[i].symbol].moreThen;
        // variableHistory.lessThen = pairs[lastPrices[i].symbol].lessThen;
      }
      
      console.log('ПАРА', pairs[lastPrices[i].symbol]);
      // variableHistory.para = pairs[lastPrices[i].symbol];
...
const preparingOrder = (currentPair, side, newPrice, minPrice, lastOrderPrice) => {
  ...
  // variableHistory.side2 = side;
  console.log('pair - ', pair)
  // variableHistory.pair2 = pair;
  console.log('balance - ', balance)
  // variableHistory.balance2 = balance;
  console.log('moreThen - ', moreThen)
  // variableHistory.moreThen2 = moreThen;
  console.log('lessThen - ', lessThen)
  // variableHistory.lessThen2 = lessThen;
  

  if (moreThen) {
    console.log('MORE THEN - ACTIVE')
    // variableHistory.moreThenActive = 'MORE THEN - ACTIVE'
  ...
  if (lessThen) {
    ...
    // variableHistory.lessThenActive2 = 'LESS THEN - ACTIVE'
    if (side === 'buy') {
      console.log('if side BUY')
      // variableHistory.ifSide2 = 'if side BUY';

      balance = currency[currentPair.qoute].balance / newPrice;
      console.log('balance делим на цену', balance)
      // variableHistory.balanceDivideByPrice2 = balance;

      console.log('Ордер хистори БЫЛ', pairs[pair].orderHistoryPrice)
      // variableHistory.orderHistoryWas2 = pairs[pair].orderHistoryPrice;

      pairs[pair].orderHistoryPrice.push(newPrice);
      console.log('Ордер хистори СТАЛ', pairs[pair].orderHistoryPrice)
      // variableHistory.orderHistoryNow2 = pairs[pair].orderHistoryPrice;
    }
    if (side === 'sell') {
      console.log('if side SELL')
      // variableHistory.ifSide2 = 'if side SELL';
  ...
  // variableHistory.balanceWithCommision = quantityWithCommision;
  // variableHistory.summWasFixed = quantityWithPrecision;
  ...
  sendOrder(access_key, secret_key, {
    ...
    loadBalances().then(() => {
      ...
      // variableHistory.checkBalance = 'Обновляем балансы';

      console.log('Финиш баланс 1', currency[whatCrypto1].balance);
      // variableHistory.FINISH = currency[whatCrypto1].balance;
